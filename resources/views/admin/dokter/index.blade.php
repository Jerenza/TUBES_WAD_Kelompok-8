...
<style>
    ...
    .btn-edit {
        background: var(--telkomedika-green);
        color: #fff;
    }
    .btn-edit:hover {
        background: var(--telkomedika-green-dark);
        color: #fff;
    }
    .btn-delete {
        background: var(--telkomedika-red);
...
<div class="admin-container">
    <h1 class="admin-title">Pengelolaan Dokter</h1>
        ...
        <table style="width:100%;margin-top:1rem;background:#fff;border-radius:10px;box-shadow:0 2px 8px rgba(0,0,0,0.04);overflow:hidden;">
            ...
            <tbody>
                @foreach($dokters as $dokter)
                    <tr>
                        <td style="padding:10px;">{{ $dokter->username }}</td>
                        <td>{{ $dokter->nama }}</td>
                        <td class="aksi-cell">
                            <a href="{{ route('admin.dokter.edit', $dokter->id) }}" class="btn-action btn-edit">Edit</a>
